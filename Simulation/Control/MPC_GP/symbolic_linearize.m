...
%     twist_b = [twist_b;K1star'*obj.data.alpha{lv1}];
    ...
    g = [g;transpose(k_star{lv1})*data.alpha{lv1}(1:200)];
    %concat gp derivatives together
    dg_dx = [dg_dx;transpose(transpose(dK_dx{lv1})*data.alpha{lv1}(1:200))];
    dg_du = [dg_du;transpose(transpose(dK_du{lv1})*data.alpha{lv1}(1:200))];
