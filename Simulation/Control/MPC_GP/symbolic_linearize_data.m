...
%     dk_dv_sym_test{lv1} = jacobian(k_sym{lv1}, v);
    ...
    Linear.k_fun{lv1}=matlabFunction(k_sym{lv1}, 'Vars', {x,I,x_data}, 'File',strcat('k_fun', num2str(lv1)));
    Linear.dk_fun{lv1}=matlabFunction(dk_sym{lv1}, 'Vars', {x,I,x_data}, 'File',strcat('dk_fun', num2str(lv1)));
    Linear.dk_dphi_fun{lv1}=matlabFunction(dk_dphi_sym{lv1}, 'Vars', {x,I,x_data}, 'File', strcat('dk_dx_fun', num2str(lv1)));
    Linear.dk_dc_fun{lv1}=matlabFunction(dk_dc_sym{lv1}, 'Vars', {x,I,x_data}, 'File', strcat('dk_dv_fun', num2str(lv1)));
...
dRbc_dphi = diff(R, I(2));
Linear.Rbc_fun = matlabFunction(Rbc,  'Vars', {I});
Linear.dRbc_dphi_fun = matlabFunction(dRbc_dphi,  'Vars', {I});
%convert I to v
h = [sqrt(v(1)^2+v(2)^2); atan(v(1)/v(2));object_gp.a/2-ry];
dI_dc = jacobian(h,v);
dI_dx = jacobian(h,x);
%convert v to u
