classdef Controller < dynamicprops  
    ...
    methods
        %% Constructor
        function obj = Controller(planner, solver_flag)  
            obj.planner = planner;
            obj.solver_flag = solver_flag;
            %define weight matrices for MPC
            if obj.planner.ps.p.num_contacts==1
%Simulation
%                 obj.Q  = 10*diag([5,3,.1,0.1]);
%                 obj.Qf = 2000*diag([5,3,1,0.1]);
%                 obj.R  = .01*diag([1,10,0.5]);
%Experiments
                obj.Q = 10*diag([3,3,.1,0.]);
                obj.Qf= 2000*diag([3,3,.1,0.]);
                obj.R = .5*diag([1,1,0.01]);
            else
                obj.Q = 10*diag([1,1,1,0.1]);
                obj.Qf= 2000*diag([1,1,1,0.1]);
                obj.R = 1*diag([1,1,1,1,0.01]);
% 
%                 obj.Q = 10*diag([1,1,.1,0.0]);
%                 obj.Qf= 1000*diag([1,1,.1,0.0]);
%                 obj.R = 0.01*diag([1,1,1,1,0.1]);
            end
...
%             clear obj.Opt
    ...
    %             H(Opt.vars.region.i(2:3,obj.mode_index_vec(lv1)), Opt.vars.region.i(2:3,obj.mode_index_vec(lv1))) = eye(10);
        ...
        function Opt = addMotionConstraints(obj, Opt, lv1, xcStar, ucStar)
            ...
            A_nom = obj.planner.ps.A_fun(xcStar, ucStar);
            B_nom = obj.planner.ps.B_fun(xcStar, ucStar);
            A_bar = eye(obj.planner.ps.num_xcStates)+obj.h*A_nom;
            B_bar = obj.h*B_nom;
