...
def getAveragedFT():
    ...
    for i in xrange(0,nsample):
        tmpft = ftmsg2list(ROS_Wait_For_Msg('/netft_data', geometry_msgs.msg.WrenchStamped).getmsg())
...
def main(argv):
    ...
    while True:
        ft, std = getAveragedFT()
        print '[ft] [%.3f %.3f %.3f %.3f %.3f %.3f]' % tuple(ft), norm(ft[0:2]), std
