classdef QlearnPend < dynamicprops
    ...
    properties (Constant)
        ...
        maxEpi = 100000; % Each episode is starting with the pendulum down and doing continuous actions for awhile.
        ...
        dt = 0.1; % Timestep of integration. Each substep lasts this long
    ...
    methods
        ...
        function obj = QlearnPend(MDP)
            obj.MDP = MDP;
            obj.x{1} = linspace(0, 0.25, 9);
            obj.x{2} = linspace(-0.2, 0.2, 9);
            obj.x{3} = linspace(-0., 0.25, 9);
            obj.x{4} = linspace(-0.2-.09/2, 0.2-.09/2, 9);
            obj.a{1} = linspace(0.0, 0.1, 3);
            obj.a{2} = linspace(-.05,.05,3);
        end
        
        
        function sIdx = get_state_index(obj, z1)
            dif = obj.states - repmat(z1,[size(obj.states,1),1]);
            row_sum = sum(dif,2);
            [~,sIdx] = min(abs(row_sum));
        end
        ...
        function [T, aIdx] = get_action(obj,Q, sIdx, episodes)
            if (rand()>obj.epsilon || episodes == obj.maxEpi) && rand()<=obj.successRate % Pick according to the Q-matrix it's the last episode or we succeed with the rand()>epsilon check. Fail the check if our action doesn't succeed (i.e. simulating noise)
                ...
                disp('***************');
                aIdx = randi(length(obj.actions)); % Random action!
            end
        ...
        R = obj.MDP.get_reward(obj.states(:,1),obj.states(:,2),obj.states(:,3), xf(1), xf(2), xf(3)); % Initialize the "cost" of a given state to be quadratic error from the goal state. Note the signs mean that -angle with +velocity is better than -angle with -velocity
...
%         [f, panel, pathmap, map, Vorig] = obj.MDP.initialize_plot(Q, R, obj.states, obj.x{1}, obj.x{2});
        ...
        for episodes = 1:obj.maxEpi
            ...
            for g = 1:obj.maxit
                ...
                T
                %simulate dynamics
                z2 = obj.MDP.get_next_state(z1, T, obj.dt, obj.substeps);
                %% UPDATE Q-MATRIX
                [bonus, success] = obj.MDP.get_terminal_state(z2, xf, obj.winBonus);
...
%                 obj.MDP.update_plot(z1, obj.x{1}, obj.x{2}, snewIdx,  Q, Vorig, episodes, f, pathmap, map, obj.transpMap, obj.doVid)
                ...
                if norm(z1(1)-z1(3))>0.1 || norm(z1(1))>1
                    break
                end
                if success
                    break;
