classdef Pendulum < dynamicprops
    %TODO: Add 
    properties (Constant)
        a_length = 1;
        x_length = 2;
    ...
    methods
        ...
        function reward = get_reward(obj, x, xdot)
            reward = (-(abs(x)).^2 + -0.25*(abs(xdot)).^2);
        end
        ...
        function zdot = Dynamics(obj, z, T)
        ...
        function [f, panel, pathmap, map, Vorig] = initialize_plot2(obj, Q, R, states, x1, x2)
        ...
        function obj = initialize_plot(obj)
            %% Set up the pendulum plot
            panel = figure;
            panel.Color = [1 1 1];
            hold on
            % Axis for the pendulum animation
            obj.f = plot(0,0,'b','LineWidth',10); % Pendulum stick
            axPend = obj.f.Parent;
            axPend.XTick = []; % No axis stuff to see
            axPend.YTick = [];
            axPend.Visible = 'off';
        ...
        function obj = update_plot(obj, x)
            set(obj.f,'XData',[0 -sin(x(1))]);
            set(obj.f,'YData',[0 cos(x(1))]);
            drawnow;
        end
        
        function obj = update_plot2(obj, z1, x1, x2, snewIdx,  Q, Vorig, episodes, f, pathmap, map, transpMap, doVid)
                            %% UPDATE PLOTS
                if episodes>0
                    % Pendulum state:
